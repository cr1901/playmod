...
pub fn mix_sample_for_tick<P>(buf: &mut Vec<i16>, state: &mut SampleState, sample: &SampleInfo, period: P, sample_rate: u32) where P: Into<u16> {
...
pub fn write_data<T>(output: &mut [T], channels: usize /* next_sample: &mut dyn FnMut() -> i16 */)
where
    T: cpal::Sample + From<i16>,
{
    let mut deque = BUFFER.lock().unwrap();
